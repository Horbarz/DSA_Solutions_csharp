...
namespace csharpFundamentals
{
    public class HackerRank
    {
        ...
        public static bool isPalindrome(string x)
        {
            ...
            //loop through the input str till the middle and check if first val === last val
        ...
        public static bool checkPanagram(string x)
        {
            ...
            //WriteLine(x.Where(ch => char.IsLetter(ch)));
            return x.Where(ch => Char.IsLetter(ch)).GroupBy(ch => ch).Count() == 26;
            //var nX = x.Distinct();
            // char[] charX = x.ToCharArray();
            // Array.Sort(charX);
            // var nx = charX.Distinct();
            // string px = new string(nx.Take(nx.Count()).ToArray());
            // WriteLine(px.Replace(" ", string.Empty));
            // WriteLine(alpha);
            // return (px.Equals(alpha));
